...
export function traverseTree (tree = [], callback, childrenTrack = 'childrenList') {
  const q = []
  tree.forEach(item => q.push({ parent: null, ...item }))
  while (q.length) {
    const item = q.shift()
...
export function findParentInTree (
  ...
  return traverseTree(tree, _item => {
    if (_item[childrenTrack].find(i => check(i, item))) {
      return _item
    }
  })
}

export function findAncestorInTree (
  tree,
  item,
  childrenTrack = 'childrenList',
  check = (a, item) => a.id === item.id
