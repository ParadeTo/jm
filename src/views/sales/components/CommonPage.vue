<template>
  ...
  <div class="app-container">
    <el-form inline :model="formModel" label-position="right" label-width="100px">
      ...
      <el-form-item label="SKU数：">{{products.length}}</el-form-item>
      <el-form-item label="总数：">{{quantitys}}</el-form-item>
      <el-form-item label="总金额：" v-if="forOrder">{{amount}}</el-form-item>
    </el-form>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="商品条码">
              <el-input v-model="productFormModel.barcode" @keydown.enter.native="searchProduct" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="商品品牌">
              <el-input readonly v-model="productFormModel.brandName" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="商品品牌">
              <el-input readonly v-model="productFormModel.productName" />
            </el-form-item>
          </el-col>
        </el-row>
        <el-row>
          <el-col :span="8">
            <el-form-item label="数量">
              <el-input type="number" min="1" v-model="productFormModel.quantity" />
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="售价">
              <el-input readonly v-model="productFormModel.price" />
            </el-form-item>
          </el-col>
          <el-col :span="4" :offset="1">
            <el-button type="primary" @click="addToProducts">添加</el-button>
          </el-col>
        </el-row>
      </el-form>
    </el-card>
    ...
    <el-card style="margin-top: 20px;">
      <el-table :data="products" element-loading-text="loading" border fit highlight-current-row>
...
export default {
  ...
  methods: {
    // {"id":null,"productId":121,"productName":"sadg","cateId":5,"cateName":"食品","productType":2,"productTypeName":null,"owncode":"sdg","classifyId":20,"classifyName":"零食子2","brandId":27,"brandName":"康师傅","unitId":14,"unitName":"包","attrValue":"","images":"","remark":null,"skuId":335,"skuName":"康师傅sadgnull","barcode":"sadg","price":123.0000}
    async searchProduct(e) {
      ...
      if (Array.isArray(items) && items.length === 1) {
        this.productFormModel = items[0];
        this.productFormModel.quantity = 1;
      }
    },
    goBack() {
      this.$router.go(-1);
    ...
    async confirm() {
      ...
      if (rsp.data && rsp.data.data) {
        ...
        this.products = products.map(p => {
          const item = orderItemInfoList.find(
            item => item.productNo === p.skuId
          );
          const res = p;
    ...
    handleCustomerChange(data) {
      ...
      if (data) {
        ...
        if (!formModel.templateName)
          formModel.templateName = `${formModel.customer.name}-模板`;
      }
    },
